...
public class YolBulmaAcGozlu{
	...
	public Sehir getNext(Sehir suanki,LinkedList<Sehir>visited,LinkedList<Sehir>tum_sehirler){
		...
		next=new YolBulmaAcGozlu().getForName(suanki.bagli_sehirler.get(i).name, tum_sehirler);
		//bu þehir daha onceden ziyaret edilmiþse 1. sehri al
		if(new YolBulmaAcGozlu().is_visited(next, visited))
			next=new YolBulmaAcGozlu().getForName(suanki.bagli_sehirler.get(i+1).name, tum_sehirler);
		for(i=1;i<suanki.bagli_sehirler.size();i++)
			if(suanki.bagli_sehirler.get(i).flight_distance_to_ankara<next.flight_distance_to_ankara &&
					!new YolBulmaAcGozlu().is_visited(suanki.bagli_sehirler.get(i), visited))
				next=new YolBulmaAcGozlu().getForName(suanki.bagli_sehirler.get(i).name, tum_sehirler);
	...
	public static void main(String[] args) {
		...
		while(true){
			Sehir next=new Sehir();
			next=new YolBulmaAcGozlu().getNext(onceki,visited,tum_sehirler);
			if(next.name=="ankara"){
		...
		System.out.println("Toplma mesafe:"+new YolBulmaAcGozlu().getSumDistance(visited));
